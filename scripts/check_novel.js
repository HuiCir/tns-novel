...
for (const file of required) {
  ...
  if (text.length < 40) failures.push(`${file} is too thin (${text.length} chars)`);
}

const chapterFiles = Array.from(
  { length: 5 },
  (_, i) => `draft/chapters/chapter-${String(i + 1).padStart(2, "0")}.md`
);
...
for (const file of existingChapters) {
  ...
  const isTemplate = text.includes("<!-- Replace this template");
  const minLen = isTemplate ? 200 : 900;
  if (text.length < minLen) {
    failures.push(`${file} is too short (${text.length} chars, min ${minLen})`);
  }
}
...
const summaries = existsSync("story_bible/chapter_summaries.md")
  ...
  : "";
const isSummaryTemplate = summaries.includes("Chapter XX") || summaries.includes("Replace this");
if (!isSummaryTemplate) {
  for (const file of existingChapters) {
    const chapterNumber = file.match(/chapter-(\d+)/)?.[1];
    if (chapterNumber && !summaries.includes(`Chapter ${chapterNumber}`) && !summaries.includes(`Chapter 0${chapterNumber}`)) {
...
console.log(
  `novel check passed: ${existingChapters.length} chapter file(s), ${required.length} story bible file(s)`
);
